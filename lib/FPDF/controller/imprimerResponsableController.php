...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"])) {
    ...
    try {
        ...
        FROM personne 
        JOIN responsable ON personne.id =  responsable.id WHERE personne.id = ? ";
        $stm = $pdo->prepare($sql);
        $params = [$id];
        $stm->execute($params);
    ...
    $tele = $responsable["Tele"];
    $dateN = (new DateTime($responsable["DateNaissance"]))->format('d-m-Y');
    ...
    $pdf->Cell(0, 10, 'Fiche Responsable', 'B', 1, 'C');
    ...
    $pdf->SetFont('', 'BI');
    $pdf->Write($h, 'Responsable' . "\n");
    $pdf->Ln(5);

    $pdf->SetFont('Arial', '', 12);
    $pdf->Write($h, $retrait . "Ajout du Responsable par l'Administrateur : ");
    $pdf->SetFont('', 'BIU');
