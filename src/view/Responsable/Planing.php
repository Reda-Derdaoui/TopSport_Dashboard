                ...
                <div class="flex flex-col  gap-5 mt-5 p-5 rounded-xl bg-white shadow-md border border-slate-100 w-250">
                    ...
                    <form class="w-50 flex items-center gap-5" action="Planing.php" method="POST">
                        ...
                        <div class="flex flex-col gap-2 ">
                            ...
                            <input class="border border-slate-300 rounded-xl p-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" type="text" id="coach" name="coach" placeholder="entraineur">
                        </div>

                        <div class="flex flex-col gap-2 ">
                            <label class="font-semibold text-slate-700 text-sm" for="jour">Jour activité: </label>
                            <input class="border border-slate-300 rounded-xl p-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" type="text" id="jour" name="jour" placeholder="jour activite">
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="font-semibold text-slate-700 text-sm" for="time">Heure activité:</label>
                            <input class="border border-slate-300 rounded-xl p-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" type="time" name="time" id="time">
                        </div>

                        <div class="flex flex-col gap-2">
                            <h1 class="font-semibold text-slate-700 text-sm">Activites: </h1>
                            <select class="cursor-pointer border border-slate-300 rounded-lg w-30 p-1 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition bg-white" name="activity" id="activity">
                    ...
                    <?php foreach ($plannings as $row): ?>
                        ...
                        <?php if ($currentDay !== $day): ?>
                            ...
                            <div class="mt-6 first:mt-0">
                                <h2 class="bg-slate-800 text-white px-6 py-2 rounded-lg text-sm font-bold uppercase tracking-widest shadow-sm">
                                    <?= htmlspecialchars($currentDay) ?>
                                </h2>
                            </div>
                        <?php endif; ?>

                        <div class="flex items-center justify-between bg-white p-5 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow ">
                            <div class="flex items-center gap-3 w-40">
                                <img class="h-5" src="../assets/icons/time-outline.svg" alt="time">
                                <span class="font-bold text-slate-700">
                                    <?= date('H:i', strtotime($row['heureDebut'])) ?> - <?= date('H:i', strtotime($row['heureFin'])) ?>
                                </span>
                            </div>

                            <div class="flex-1 px-4">
                                <span class="bg-sky-100 text-sky-700 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                            ...
                            <div class="flex gap-2 item-center ml-20">
                                <button data-id="<?= $row["Id_Planning"] ?>"
                                    data-name="<?= $row['activity_name'] ?>"
                                    data-day="<?= $row['jour'] ?>"
                                    data-time="<?= $row['heureDebut'] ?>"
